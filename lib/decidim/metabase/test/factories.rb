...
FactoryBot.modify do
  factory :organization, class: "Decidim::Organization" do
    ...
    metabase_configuration { nil }

    trait :secure_context do
      host { "localhost" }
    end

    trait :with_metabase_configuration do
      metabase_configuration do
        login { Faker::Lorem.characters(number: rand(1..10)).to_s.gsub("'", "_") }
        password { Faker::Lorem.characters(number: rand(1..10)).to_s.gsub("'", "_") }
        dashboard_ids { generate(:dashboard_ids) }
      end
    end
